...
Promise.all([d3.csv("../datasources/eurostat-2019-cleaned.csv", null, data => ({
  ...
  const electricVehicleProductionEmissionsPerKm = 38; // % Vehicle production emissions: 3.9-5.7 4.8 (median) (tonne CO\textsubscript{2}e/kg) 38 (g CO\textsubscript{2}e/km) 
  ...
  const conventionalVehicleProductionEmissionsPerKm = 46; // Average electric vehicle battery guarantee (km)

  const averageElectricVehicleBatteryLifespan = 125000 * 1.60934; // Average electric vehicle battery capacity (kWh)
  ...
  const bars = svg.select(".plot-area").selectAll("g.series").data(data).join("g").classed("series", true).classed("hidden", false).attr("id", d => "b" + d.key).style("fill", d => colorScale(d.key)).on("mouseover", (e, d) => {
  ...
  }).selectAll("rect").data(d => d).join("rect").attr("width", xScale.bandwidth()).attr("y", d => yScale(d[1])).attr("x", d => xScale(d.data.type)).attr("height", d => yScale(d[0]) - yScale(d[1]) || 0);
