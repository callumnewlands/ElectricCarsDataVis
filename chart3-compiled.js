...
Promise.all([d3.json("../datasources/uk.json"), d3.csv("../datasources/worldcities.csv", null, data => ({
  ...
  categories.forEach((r, i) => {
    svg.append("circle").attr("cx", width - legendWidth - (r.area ? 20 : 0)).attr("cy", legendYOffset + (r.area ? 20 : 0) + 20 * i).attr("r", r.area ? 24 : 6).style("stroke", r.area ? r.colour : null).style("fill", r.colour + (r.area ? "33" : ""));
    svg.append("text").attr("x", width - legendWidth + 10).attr("y", legendYOffset + (r.area ? 17 : 0) + 1 + 20.5 * i).text(r.name).attr("class", "legendMark").attr("alignment-baseline", "middle");
  });
});
